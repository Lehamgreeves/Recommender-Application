...
with tab1:
    ...
    mode = st.radio("Recommendation Mode:", ["User Preference Matching", "Similar Destination Search"])
    
    if mode == "User Preference Matching":
        c1, c2, c3 = st.columns(3)
        pref_type = c1.selectbox("Preferred Scenery", options=["beach", "city", "mountain"])
        max_budget = c2.slider("Max Budget ($)", 500, 10000, 2000, 100)
        pref_days = c3.slider("Trip Duration (Days)", 1, 30, 7)
        
        if st.button("Find Matching Destinations"):
            user_input = pd.DataFrame([{
        ...
        target_dest = st.selectbox("Select Target Destination:", options=destination_profiles['Destination'].unique())
...
with tab2:
    ...
    selected_traveler = st.selectbox("Select Traveler Name:", options=user_item_matrix.index)
